...
class App {
  ...
  handleSelect() {
    ...
    this.medias.forEach((media) => {
      if (
        Math.abs(media.plane.position.x) <
        Math.abs(closest.plane.position.x)
      ) {
        closest = media;
      }
    });
    ...
    if (this.onSelect) {
      this.onSelect(
        closest.index % (this.medias.length / 2)
      );
    }
  }
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onSelect]);
  return (
    <div
